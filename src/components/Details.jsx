...
// Component that renders weather details and forecast data
export const Deatils = () => {
    ...
    // Function to handle syncing of data
    const handleSyncData = () => {
        setIsRotate(true);
        dispatch(syncData(data.name, toast))
    }

    // Rendering weather details and forecast data
    return isLoading ? (
        <Loading />
    ) : isError ? (
        <Error />
        ...
        <>
            <Box maxW={'1400px'} m={'20px auto 5px'} p={'20px'} minH={'550px'}>
                <Grid gridTemplateColumns={['100%', 'repeat(2, 1fr)', 'repeat(2, 1fr)', '30% 27.5% 38%']} gap={'30px'}>
                    <Newbox>
                        <Box color={'#008B8B'} p={'20px'} textAlign={'center'}>
                            <Flex justify={'end'}>
                                {/* // Icon to trigger data syncing */}
                                <Icon
                    ...
                    <Newbox>
                        <Grid templateColumns={'50% 50%'} h={'100%'} p={'8px'}>
                            ...
                            <Box borderRadius={'30px'} bg={'#008B8B'} py={'10px'} pl={'15%'}>
                ...
                <div className="tagLine">
                    <p id="madeByVipul">Made with ❤️ by Vipul Kumar</p>
                </div>
            </Box >
        </>
    );
